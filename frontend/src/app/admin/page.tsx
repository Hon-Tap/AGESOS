...
export default function AdminDashboard() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (method === 'POST') {
          ...
          setNews(news.map(n => n.id === savedItem.id ? savedItem : n));
        }
        setIsEditing(false);
  ...
  if (isLoading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-white">
      ...
      <p className="mt-4 font-bold text-[#1F305E]">Loading AGE Command Center...</p>
    </div>
  );

  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden">
      {/* SIDEBAR - Fixed height, no overlap */}
      <aside className="w-64 bg-[#1F305E] text-white flex flex-col z-50 shadow-2xl">
        <div className="p-8">
          <div className="flex items-center gap-3 mb-12">
            ...
            <div>
              <h2 className="text-sm font-bold">AGE ADMIN</h2>
              <p className="text-[#60A0D2] text-[8px] font-black tracking-[0.2em]">SOUTH SUDAN</p>
            </div>
          </div>
      ...
      {/* MAIN VIEW - Scrollable independently */}
          ...
          {activeTab === 'overview' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="mb-10">
                ...
                <p className="text-slate-500">System status and impact metrics.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 group hover:border-[#60A0D2] transition-all">
                  <Newspaper className="w-8 h-8 text-blue-500 mb-6" />
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Updates</p>
                  <h3 className="text-5xl font-black text-[#1F305E]">{stats.totalUpdates}</h3>
                </div>
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                  <Mail className="w-8 h-8 text-red-500 mb-6" />
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Unread Inbox</p>
                ...
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Sectors</p>
              ...
              {isEditing ? (
                <div className="bg-white p-8 rounded-3xl border-2 border-[#60A0D2] shadow-2xl animate-in zoom-in-95">
                  <div className="flex justify-between mb-8">
                    <button onClick={() => setIsEditing(false)} className="text-slate-400 hover:text-slate-600 flex items-center gap-2">
                  ...
                  <form onSubmit={handleSaveNews} className="grid grid-cols-2 gap-6">
                    ...
                    <div className="col-span-2">
                      <label className="text-[10px] font-black uppercase text-slate-400 block mb-2">Detailed Report</label>
                      <textarea 
                        rows={6}
                    ...
                    <div className="col-span-2 flex justify-end gap-4">
                      <button type="button" onClick={() => setIsEditing(false)} className="font-bold text-slate-400">Cancel</button>
                      <button type="submit" className="bg-[#1F305E] text-white px-10 py-4 rounded-xl font-black flex items-center gap-2">
                ...
                <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden">
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 text-[10px] font-black uppercase tracking-widest text-slate-400">
                      <tr>
                        <th className="px-8 py-5">Headline</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {news.map(item => (
                        <tr key={item.id} className="group hover:bg-slate-50">
                          ...
                          <td className="px-8 py-6 text-sm font-medium text-slate-500">{item.category}</td>
                          <td className="px-8 py-6">
                          ...
                          <td className="px-8 py-6 text-right">
                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => { setCurrentPost(item); setIsEditing(true); }} className="p-2 text-slate-400 hover:text-blue-500"><Edit3 className="w-4 h-4" /></button>
                              <button onClick={() => deleteNews(item.id)} className="p-2 text-slate-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {activeTab === 'inquiries' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {inquiries.map(inq => (
                  <div key={inq.id} className="bg-white p-6 rounded-2xl border border-slate-100 flex items-center justify-between group hover:shadow-lg transition-all">
                    ...
                    <p className="text-slate-500 text-sm italic max-w-md truncate">&ldquo;{inq.message}&rdquo;</p>
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] font-black text-slate-300 uppercase">{inq.date}</span>
                      <button className="bg-[#1F305E] text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase">View</button>
                    </div>
                  </div>
                ))}
              </div>
